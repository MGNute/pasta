...
def pasta_postproc_step2(alignment_as_tmp_filename_to_report, job, pasta_products, restore_to_rna,
                         tree_as_tmp_filename_to_report, user_config):
    #######################################################################
...
def pasta_postproc_step1(alignment_as_tmp_filename_to_report, job, options, pasta_config, pasta_config_dict,
                         pasta_products, pasta_team, temporaries_dir, tree_as_tmp_filename_to_report, user_config):
    global _RunningJobs
    ...
    if user_config.commandline.raxml_search_after:
        ...
        _RunningJobs = None
        tree_as_tmp_filename_to_report = pasta_products.get_abs_path_for_iter_output("postraxtree", TEMP_TREE_TAG,
                                                                                     allow_existing=True)
    ...
    return alignment_as_tmp_filename_to_report, tree_as_tmp_filename_to_report


def pasta_prelim_step3(multilocus_dataset, options, pasta_config, pasta_products, pasta_team, score,
                       alignment_as_tmp_filename_to_report, starting_tree, temporaries_dir,
                       tree_as_tmp_filename_to_report, tree_file):
    '''
    creates the initial alingment and tree
    '''
    global _RunningJobs
    ...
    return alignment_as_tmp_filename_to_report, pasta_config_dict, score, starting_tree_str, tree_as_tmp_filename_to_report


def pasta_prelim_step2(multilocus_dataset, options, pasta_products, user_config):
    '''
    relabels taxa if they have bad names.
    '''
    ############################################################################
    ...
    return prev_signals, restore_to_rna


def pasta_prelim_step1(multilocus_dataset, temporaries_dir, user_config):
    '''
    Broadly speaking, this subroutine starts the threads, reads the tree andinitializes some variables.
    ...
    return alignment_as_tmp_filename_to_report, options, pasta_config, score, starting_tree, tree_as_tmp_filename_to_report, tree_file
...
def run_pasta_from_config(user_config, pasta_products, multilocus_dataset):
    ...
    if cmdline_options.interruptible==True:
        if cmdline_options.keeptemp==False:
            MESSENGER.send_info("WARNING: Over-riding option keeptemps=False because interruptible has been set to True. The temproary files are necessary for the intermediate job to run.")
            cmdline_options.keeptemp=True
        cmdline_options.keepalignmenttemps=True
